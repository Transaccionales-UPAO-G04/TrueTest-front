...
export class TestComponent implements OnInit {
  ...
  handleAnswer(puntuacion: number): void {
    ...
    if (this.currentStep < this.questions.length - 1) {
      ...
      // Debugging: Manually set result if not set
      if (!this.result) {
        console.error('Result not calculated correctly');
        const defaultArea = 'Ciencias'; // Choose a default area
        this.result = {
          ...this.areas[defaultArea],
          scores: [],
    ...
    try {
      ...
      if (this.answers.length !== this.questions.length) {
        console.error('Not all questions answered', 
          `Answers: ${this.answers.length}, Questions: ${this.questions.length}`);
        
        // Force complete answers if some are missing
        this.questions.forEach(question => {
          if (!this.answers.some(a => a.idPregunta === question.id)) {
            this.answers.push({
              idPregunta: question.id,
              puntuacion: 3, // Neutral score
              area: question.area
            });
          }
        });
      ...
      this.answers.forEach(answer => {
        ...
        console.log('Intermediate Area Scores:', areaScores);
      });

      // Normalize scores
      this.areaScores = Object.entries(areaScores).map(([area, score]) => {
        const questionCount = this.questions.filter(q => q.area === area).length;
        const normalizedScore = (score / (questionCount * 5)) * 100;
        
        console.log(`Area: ${area}, Raw Score: ${score}, Normalized Score: ${normalizedScore}`);
      ...
      this.areaScores.sort((a, b) => b.value - a.value);

      // Get top area
      const topArea = this.areaScores[0].area;

      console.log('Top Areas:', this.areaScores);
      console.log('Chosen Top Area:', topArea);
      ...
      console.log('Final Calculated Result:', this.result);
